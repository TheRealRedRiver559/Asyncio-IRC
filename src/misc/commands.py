...
from misc.utilities import user_id_from_username
from misc.storage import clients, banned_users, channels, reload_event
...
async def send_command_details(client: Client) -> None:
    ...
    for command_name, command_data in Commands.commands.items():
        if command_data[4]:
    ...
    data = {"prefix": Commands.prefix, "slash_commands": available_slash_commands}
...
async def help_text(client: Client) -> str:
    ...
    available_command_names = list(
        filter(
            lambda command_name: int(Commands.commands[command_name][1])
            <= int(client_permission_level),
            Commands.commands,
        )
    )
    for command_name in available_command_names:
        command_description = Commands.commands[command_name][3]
        command_level = Commands.commands[command_name][1]
        description_text += f"{Commands.prefix}{command_name}:(Lvl:{command_level}) -- {command_description}\n"
...
async def execute_command(message: Message) -> None:
    ...
    valid_command = Commands.commands.get(function_name, False)
    ...
    show_command_usage = Commands.commands[function_name][2]
