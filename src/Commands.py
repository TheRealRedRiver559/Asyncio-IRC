...
async def check_command(client, data: str):
    try:
        ...
        message = data.removeprefix(Commands.prefix).split()  # removes the command prefix from the message
...
async def join_channel(client, channel_name):
    ...
    if channel_name in channels.keys():
        ...
        if client in channel.clients:
            ...
            await send_data(client, {"sender": "Server", "message": channel_name, "message_type": "CHANNEL_CHANGE"})
...
async def leave_channel(client):
    ...
    await send_data(client, {"sender": "Server", "message": None, "message_type": "CHANNEL_CHANGE"})
    return format
    #await send_data(client, format)
...
async def change_channel_perm(*data):
    ...
    format["message"] = f"{target_username} has had their permissions set to {permission_level}."
    return format
    #await send_data(client, format)
