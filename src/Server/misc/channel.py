...
class Channel:
    ...
    async def _add_user(self, *new_clients: tuple):
        ...
        defualt_permission_level = 5 # this is just for testing. All new users added to channels will have max perms. 
        ...
        for client in new_clients:
            ...
            if not channel_record:
                cursor.execute("INSERT INTO UserChannelPermissions (user_id, channel_id, permission_level) VALUES (?, ?, ?);", (client.id, self.channel_id, defualt_permission_level,))
                conn_changes = True
    ...
    async def check_ban(self, client: Client) -> dict:
...
def generate_private_channel_tag(user_ids: list) -> str: # Very weird id system, but it works for now.
...
class PrivateMessageChannel(Channel): # Private channel. WIP
    # TODO finish this :/
